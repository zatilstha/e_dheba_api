	...
	public function rate(Request $request, $newRequest ) {
		try {
			...
			$newRequest->update(['user_rated' => 1]);
			 ...
			 /*--------MLM----------*/
			 $user_lists=array();
			 $user_commisions=array();
			 $user=User::find($newRequest->user_id);
			 $RequestPayment=RideRequestPayment::where('ride_request_id',$newRequest->id)->first();
			 if($user->referral_status != 1){
			 	$user_lists['A']=$user->Areferral_userid;
			 	$user_lists['B']=$user->Breferral_userid;
			 	$user_lists['C']=$user->Creferral_userid;

			 	$user_commisions['A']=$RequestPayment->total*($this->settings->site->A_referral/100);
			 	$user_commisions['B']=$RequestPayment->total*($this->settings->site->B_referral/100);
			 	$user_commisions['C']=$RequestPayment->total*($this->settings->site->C_referral/100);

			 	//for create the user wallet transaction
			 	foreach ($user_lists as $key => $user_list) {
			 		if ($user_list) {
			 			$transaction['amount']=$user_commisions[$key];
						$transaction['id']=$user_lists[$key];
						$transaction['transaction_id']=$newRequest->id;
						$transaction['transaction_alias']=$newRequest->booking_id;
						$transaction['company_id']=$newRequest->company_id;

						//CBS Wallet Transaction
						$newuser=User::find($user_lists[$key]);
			            $siteConfig = $this->settings->site;
			            $fromClientId = "6575";
			            $fromAccountId = "4988";
			            $toClientId = $newuser->client_id;
			            $toAccountId = $newuser->account_id;
			            $wallet_Response = (new Transactions)->walletcharging_api($user_commisions[$key],$fromClientId,$fromAccountId,$toClientId,$toAccountId,$siteConfig);
						...
						(new Transactions)->userCreditDebit($transaction,2);
			 		}
