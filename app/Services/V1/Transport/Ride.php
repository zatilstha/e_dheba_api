...
\Log::info("taxi ride------"); \Log::info($request->all());
...
\Log::info("count of provider-----".count($Providers)); \Log::info($Providers);		
...
$rideRequest->est_fare = $request->est_fare ? $request->est_fare  : "";
				...
				$rideRequest->someone_email=$request->someone_email;
				 try{
					  if(!empty($request->someone_email)&&!empty($this->settings->site->send_email) && $this->settings->site->send_email == 1 && $this->settings->transport->ride_otp) {
					    ...
					    {
					    	 $plusCodeMobileNumber=$request->someone_mobile;
			...
			if(in_array($rideRequest->status, ['SEARCHING','STARTED','ARRIVED','SCHEDULED'])) {
				...
				if($request->cancelled_by == "PROVIDER") {
					if($this->settings->transport->broadcast_request == 1){
						...
						(new SendPushNotification)->ProviderCancelRide($rideRequest->user_id,'transport');
	...
	public function invoice($request_id, $toll_price = 0)
	{
		try {                      
			...
			if($rideRequest->use_wallet == 1 && $payable_amount > 0){
				...
				$fromClientId = $User->client_id;
				$fromAccountId = $User->account_id;
				$toClientId = 6005;
	            $toAccountId = 4542;
				...
				if($Wallet != 0){
					...
					if($payable_amount > $Wallet) {
						//CBS Wallet Transaction
						$wallet_Response = (new Transactions)->walletcharging_api($Wallet,$fromClientId,$fromAccountId,$toClientId,$toAccountId,$siteConfig);
						if(@$wallet_Response['savingsId']){
						...
						//CBS Wallet Transaction
						$wallet_Response = (new Transactions)->walletcharging_api($payable_amount,$fromClientId,$fromAccountId,$toClientId,$toAccountId,$siteConfig);
						if(@$wallet_Response['savingsId']){
							$Payment->payable = 0;
