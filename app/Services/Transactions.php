...
class Transactions{
    ...
    protected function createUserWallet($request){
        ...
        $userWallet->save();
        \Log::info("User wallet added before ------".$user->wallet_balance);

        //update the user wallet amount to user table        
        $user->wallet_balance=$user->wallet_balance+($request['amount']);
        $user->save();

        \Log::info("User wallet added after ------".$user->wallet_balance);
    ...
    public function referralCreditDebit($request,$type=1){
        ...
        if($type==1){
            \Log::info("referal transaction user========");
    ...
    public function walletcharging_api($payable_amount,$fromClientId,$fromAccountId,$toClientId,$toAccountId,$siteConfig=1){
