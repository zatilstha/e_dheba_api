    ...
    public function create_referral($referral_code, $referral_data, $settings, $referrer)
    {
        ...
        if(!empty($referral)){
            ...
            if($User){
                ...
                \Log::info("Referal count--------".$referral->referal_count);

            }

            if($siteConfig->referral_amount >0){

                \Log::info("Referal amount----".$siteConfig->referral_amount);
                ...
                if($referrer == "user") {
                   ...
                   \Log::info("referal count======".$Referralcount);
                } else if($referrer == "provider") {
                ...
                if(($siteConfig->referral_count >0) && ($siteConfig->referral_count <= $Referralcount)){
                    ...
                    $newuser=User::find($referral->id);
                    $siteConfig = $siteConfig;
                    $fromClientId = "6575";
                    $fromAccountId = "4988";
                    $toClientId = $newuser->client_id;
                    $toAccountId = $newuser->account_id;
                    $wallet_Response = (new Transactions)->walletcharging_api($siteConfig->referral_amount,$fromClientId,$fromAccountId,$toClientId,$toAccountId,$siteConfig);
