	...
	try{	
		...
		if($provider == 'user')  {
			...
			if($user){
				...
				$data['password']=$request->social_unique_id;			

				$CBSlogin = Helper::CBSselflogin($data);

				return Helper::getResponse(['data' => ["token_type" => "Bearer", "expires_in" => (config('jwt.ttl', '0') * 60), "access_token" => $token, 'user' => $user,'CBSlogin' => $CBSlogin]]);
			}else{
