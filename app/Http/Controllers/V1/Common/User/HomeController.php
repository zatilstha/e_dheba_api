...
class HomeController extends Controller
{
    ...
    public function menus(Request $request) {
        ...
        $menus->services = AdminService::with('menu')
            ->whereHas('menu.cities', function ($query) use($user) {
	...
	public function show_profile()
	{
		...
		if($settings->site->referral==1){
            ...
            $user_details['referral']->A_referral=(int)$settings->site->A_referral;

            $user_details['referral']->B_referral=(int)$settings->site->B_referral;

            $user_details['referral']->C_referral=(int)$settings->site->C_referral;
		}
    ...
    public function listpromocode($service=null) {
        ...
        $promocodes = Promocode::where('company_id', $company_id)->where('service',$type)->whereDate('expiration','>=',Carbon::today())->get();
    ...
    // public function debit_amount(){

    //     $fromClientId = "6575";
    //     $fromAccountId = "4988";
    //     $toClientId = "1588";
    //     $toAccountId = "1390";
    //     $referral_amount = "540.00";
    //     $settings = json_decode(json_encode(Setting::where('company_id', 1)->first()->settings_data));
    //     $siteConfig = $settings->site;

    //     $wallet_Response = (new Transactions)->walletcharging_api($referral_amount,$fromClientId,$fromAccountId,$toClientId,$toAccountId,$siteConfig);
